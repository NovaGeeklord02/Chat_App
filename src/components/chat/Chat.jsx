...
const Chat = ({ showDetail, setShowDetail }) => {
  ...
  useEffect(() => {
    const unSub = onSnapshot(doc(db, "chats", chatId), (res) => {
      setChat(res.data());
      setCallerUserID(String(currentUser.id)); // Current user is caller
      setCalleeUserID(String(user.id));  // Chat recipient is callee
      
      // Debug log
      console.log("Setting up call IDs:", {
  ...
  useEffect(() => {
    if (chatId && user && currentUser) {
      // Get the cleanup function from changeChat
      const cleanup = useChatStore.getState().changeChat(chatId, user);
  ...
  const init = async () => {
    try {
        console.log("Initializing with userID:", callerUserID);
  ...
  const fun1 = async () => {  // Audio call
    try {
        await init();
        console.log("Initialized with caller:", callerUserID, "callee:", calleeUserID);
        
        const enable = true;
        await TUICallKitServer.enableFloatWindow(enable);
        await TUICallKitServer.enableVirtualBackground(enable);
